---
title: "Training and visusualisation of AUS to landmarks and delaunay faces"
...
ExportedSets subdirectory changed to au-to-land-align
```{r}
...
multiple videos (this doesn't quite align yet)
```{r}
...
this (up) doesn't quite align but we can take the data from here
```{r}
...
```
allign to reference frame
this is the bit the actually alligns landmarks
...
# set wd to HaloStudy/Data/ExportedSets/ProcAllign/allignedtoneutral"
...
# this is actually alligned now

```

import the videos back from the step above (i.e transformed)
```{r}
...
# this compares data before alligment and after alligning
# this takes too long
# dta_to_allign%>%
#   group_by(filename)%>%
...
unique(tmp_combined_alligned_to_nt_data$filename)


saveRDS(tmp_combined_alligned_to_nt_data,
                 file.path(paths$au_to_land, 
                  "dta_pos_aligned_to_nt.rds"))
...
# can make an animation to see
test<- tmp_combined_alligned_to_nt_data %>%
  subset(as.numeric(as.factor(filename)) == 2)%>%
ggplot( aes(x = x, y = y)) + 
  geom_point() + 
  theme_minimal()+
...
saveRDS(tmp_eye_align_data_combined,
                 file.path(paths$au_to_land, 
                  "dta_pos_aligned_to_nt_and_eye.rds"))
...
tmp_eye_align_data_combined_norm_01_wide_AUS
...
saveRDS(dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs, "dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs.rds")
...
range(dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs_10th$AU26_r_Jaw_drop)

mod_pls_test <- plsr(as.formula(paste("cbind(", paste(landmark_columns, collapse = ", "), ") ~ ", paste(au_columns, collapse = " + "))),
                  data = dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs_10th,
                  ncomp = 17,
                  scale = FALSE,  # Standardize variables
                  validation = "CV")

# FOR VISUALISATIONS, MIGHTH BE BETTER TO TRAIN THIS SEPARATELLY PER EMOTION/EXPRESSIONH
...
mod_pls_ang <- plsr(as.formula(paste("cbind(", paste(landmark_columns, collapse = ", "), ") ~ ", paste(au_columns, collapse = " + "))),
                  data = dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs_10th%>%
                    subset(grepl("angry", filename)),
                  ncomp = 17,
                  scale = FALSE,  # Standardize variables
                  validation = "CV")


mod_pls_sad <- plsr(as.formula(paste("cbind(", paste(landmark_columns, collapse = ", "), ") ~ ", paste(au_columns, collapse = " + "))),
                  data = dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs_10th%>%
                    subset(grepl("sad", filename)),
                  ncomp = 17,
                  scale = FALSE,  # Standardize variables
                  validation = "CV")

mod_pls_hap <- plsr(as.formula(paste("cbind(", paste(landmark_columns, collapse = ", "), ") ~ ", paste(au_columns, collapse = " + "))),
                  data = dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs_10th%>%
                    subset(grepl("happy", filename)),
                  ncomp = 17,
                  scale = FALSE,  # Standardize variables
                  validation = "CV")
```
...
as.data.frame(mod_pls_test$model)
...
saveRDS(dta_eye_align_combined_norm_01_wide_AUS_reconstr_norm_downs_10th,
          file.path(paths$au_to_land, 
                    "dta_pos4_au_to_land_pls.rds"))



range(dta_eye_align_combined_norm_01_wide_AUS_reconstr_norm_downs_10th$AU26_r_Jaw_drop)
...
colnames(dta_eye_align_combined_norm_01_wide_AUS_reconstr_norm_downs_10th)

dta_all_aligned_landmarks_posed_corrected_AUS_norm_downs_10th[,1:138]%>%
  pivot_longer(
    cols = matches("^[xy]_"), 
    names_to = c(".value", "id"),
...
saveRDS(dta_posed_reconstr_k3,
         file.path(paths$au_to_land, 
                   "dta_posed_reconstr_k3.rds"))
...
process and plot delaunay components
```{r}
library(dplyr)
library(tidyr)
library(ggplot2)
...
# res_k3_spoken1 <- readRDS("~/HaloStudy/Data/res_k3_spoken1.rds")
...
- just rely on the above where we train the model on pure AUS
...
# now load all alligned data again
this bit not working properly - skip
```{r eval=FALSE, include=FALSE}
library(dplyr)
...
#
all_frames_transformed$filename
all_frames_transformed %>%
  ggplot(aes(x,y))+
  geom_text(aes(label = land_id))+
  scale_y_reverse()

# this works - it is alligned now
...
mod_pls_spoken_an <- plsr(as.formula(paste("cbind(", paste(landmark_columns, collapse = ", "), ") ~ ", paste(au_columns, collapse = " + "))),
                  data = dta_spk_eye_align_combined_norm_01_wide_AUS_norm_downs_10th%>%
                     subset(grepl("angry", filename)),
...
mod_pls_spoken_sad <- plsr(as.formula(paste("cbind(", paste(landmark_columns, collapse = ", "), ") ~ ", paste(au_columns, collapse = " + "))),
                  data = dta_spk_eye_align_combined_norm_01_wide_AUS_norm_downs_10th%>%
                     subset(grepl("sad", filename)),
...
mod_pls_spoken_hap <- plsr(as.formula(paste("cbind(", paste(landmark_columns, collapse = ", "), ") ~ ", paste(au_columns, collapse = " + "))),
                  data = dta_spk_eye_align_combined_norm_01_wide_AUS_norm_downs_10th%>%
                     subset(grepl("happy", filename)),
...
spoken graphs from spoken pls mopdel
...
store pngs for github
note we used the pls for spoken
```{r}
...
#                          ".RData"
save.image(paste0(paths$envRdata, 
                  "spt_tmp_emo_faces_au_to_land_vis_subm1.RData"))
