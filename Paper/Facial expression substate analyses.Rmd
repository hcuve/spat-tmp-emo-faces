...
```{r}
unique(df_with_clusters6$filename)

library(ggplot2)
library(dplyr)

# Extract the necessary features for clustering
tmp_dta <- df_with_clusters6 %>%
  subset(as.numeric(factor(filename)) == 10)%>%
  select(sp_avg, comp_delta_avg, compavg)
unique(tmp_dta$filename)


# Function to calculate WSS
wss <- function(k) {
  kmeans(tmp_dta[,2:4], centers = k, nstart = 10)$tot.withinss
}
...
ggplot(elbow_plot, aes(x = as.factor(k), y = WSS)) +
  geom_line() +
  geom_point() +
...
avg_silhouette <- function(k) {
  km <- kmeans(tmp_dta[,2:4], centers = k, nstart = 10)
  sil <- silhouette(km$cluster, dist(tmp_dta[,2:4]))
  mean(sil[, 3])
}

# Calculate silhouette scores
silhouette_values <- sapply(2:10, avg_silhouette)

# Plot the Silhouette Analysis
silhouette_plot <- data.frame(k = 2:10, Silhouette = silhouette_values)
...
tmp_tsclust_result <- dtwclust::tsclust(tmp_dta[, c("sp_avg", "comp_delta_avg", "compavg")], k = c(2,3,4), args = tsclust_args(cent = list(trace = TRUE)))



tmp_tsclust_result

sapply(tmp_tsclust_result, cvi, type = "internal")

tmp_tsclust_result2 <- dtwclust::tsclust(tmp_dta[, c("sp_avg", "comp_delta_avg", "compavg")], k = c(2,3,4,5), args = tsclust_args(cent = list(trace = TRUE)))


tmp_tsclust_result2[1]

dtwclust::cvi(tmp_tsclust_result2)
sapply(tmp_tsclust_result2, cvi, type = "internal")
sapply(df_with_clusters6$cluster_across, cvi, type = "internal")
...
tmp_dta <- df_with_clusters6 %>%
  subset(as.numeric(factor(subject)) == 10)%>%
  select(sp_avg, comp_delta_avg, compavg)
unique(tmp_dta$filename)

# Data: Assuming `tmp_dta` contains the time series data
# Specify the columns for clustering
data_to_cluster <- tmp_dta[, c("sp_avg", "comp_delta_avg", "compavg")]
...
for (k in k_values) {
  cluster_results[[as.character(k)]] <- tsclust(
    data_to_cluster,
    k = k,
...
# Calculate internal validity indices for each k
cvi_results <- sapply(cluster_results, cvi, type = "internal")

# Convert results to a dataframe for easier manipulation
cvi_df <- as.data.frame(t(cvi_results))
cvi_df$k <- k_values

# Visualise validity indices for each k
library(ggplot2)
cvi_long <- reshape2::melt(cvi_df, id.vars = "k")
ggplot(cvi_long, aes(x = k, y = value, colour = variable, group = variable)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Cluster Validity Indices Across k",
...
spoken_NMF_with_clusters6$filename

library(dtwclust)

test_cl<-spoken_NMF_with_clusters6%>%
  subset(filename == "./cut_spoken_angry_day1_p1.csv")

tmp_cl<-tsclust(test_cl[, c("sp_avg", "comp_delta_avg", "compavg")], k = c(2,3,4))
