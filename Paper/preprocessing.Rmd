...
sm_2$emot_only_downs_eg<-
df_OF_output_AUsW_unblind_posed_binned[,1:32]%>%
  subset(filename == "./cut_posed_happy_day2_p25.csv")%>%
  ...
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
      ...
      group_by(AUs)%>%
  mutate(AU.coefs = normalize_0_1(AU.coefs))%>%

  # subset(drug.placebo == "drug")%>%
    ggplot(aes(as.numeric(bin_frame), AUs, fill = AU.coefs))+
    geom_tile()+
    ggtitle("expression only - downsampled")+
     xlab("bin frame ")+
    # facet_grid(posed.spoken~expression)+
   p$graphstyle_int+
  theme(axis.text.y = element_blank())+
...
df_OF_output_AUsW_unblind_posed%>%
  ...
  # subset(drug.placebo == "drug")%>%
    ...
    geom_tile()+
     xlab("frame (24 fps)")+
    # facet_grid(posed.spoken~expression)+
  ggtitle("expression only - full samples")+
   p$graphstyle_int+
  # theme(axis.text.y = element_blank())+
  scale_fill_viridis_c(option = "magma")
...
sm_2$emotiv_speech_only_downs_eg<-
df_OF_output_AUsW_unblind_spoken1_2_binned[,1:33]%>%
    select(-success)%>%
   subset(filename == "./cut_spoken_sad_day1_p28.csv")%>%
    gather(AUs, AU.coefs, -(1:15))%>%
  group_by(bin_frame, AUs,expression)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
   group_by(AUs)%>%
  ...
  # subset(drug.placebo == "drug")%>%
   ...
   xlab("bin frame ")+
    # facet_grid(posed.spoken~expression)+
    ggtitle("emotive speech - downsampled")+
    # facet_grid(posed.spoken~expression)+
   p$graphstyle_int+
  theme(axis.text.y = element_blank())+
  scale_fill_viridis_c(option = "magma")
...
sm_2$emotiv_speech_fullsamp_eg<- 
df_OF_output_AUsW_unblind_spoken1_2%>%
   subset(filename == "./cut_spoken_sad_day1_p28.csv")%>%
  # select(-success)%>%
    gather(AUs, AU.coefs, -(1:15))%>%
  ...
  # subset(drug.placebo == "drug")%>%
    ...
    geom_tile()+
    # facet_grid(posed.spoken~expression)+
  ggtitle("emotive speech- full samples")+
  xlab("frame (24 fps)")+
   p$graphstyle_int+
  # theme(axis.text.y = element_blank())+
  scale_fill_viridis_c(option = "magma")
...
sm_2$downsam_examp<- (sm_2$emot_only_fullsamp_eg+sm_2$emot_only_downs_eg)/
(sm_2$emotiv_speech_fullsamp_eg+sm_2$emotiv_speech_only_downs_eg)+
  plot_layout(guides = "collect")

sm_2$downsam_examp

# note guides at thw top cause issues
ggsave("sm_sdownsam_examp.tiff",
       device = 'tiff', sm_2$downsam_examp,
       width = 18,
       height = 12,
       dpi = 300,
       compression = "lzw")
